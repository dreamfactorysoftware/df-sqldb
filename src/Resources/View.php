 ...
 */
class View extends BaseDbResource
{
    //*************************************************************************
    //	Constants
    //*************************************************************************
    ...
    /**
     * Resource tag for dealing with table views
     */
    const RESOURCE_NAME = '_view';
    /**
     * Replacement tag for dealing with view events
     */
    const EVENT_IDENTIFIER = '{view_name}';
    ...
    {
        $result = $this->getViews($schema, $refresh);
        $resources = [];
    ...
    protected function getViews($schema = '', $refresh = false)
    {
        if ($refresh || (is_null($views = $this->parent->getFromCache('views')))) {
            $views = [];
            ...
            foreach ($this->parent->getSchemas($refresh) as $schemaName) {
                ...
                foreach ($result as &$view) {
                    if ($addSchema) {
                        $view->name = ($addSchema) ? $view->internalName : $view->resourceName;
                    }
                    $views[strtolower($view->name)] = $view;
                }
            }
            ksort($views, SORT_NATURAL); // sort alphabetically
            $this->parent->addToCache('views', $views, true);
        }

        if (!empty($schema)) {
            $out = [];
            foreach ($views as $view => $info) {
                if (starts_with($view, $schema . '.')) {
                    $out[$view] = $info;
                }
            }
...
//        }
        ...
        if (!is_null($this->resourceId)) {
            if ($this->parent->getCacheEnabled()) {
                // build cache_key
                $cacheKey = strtolower($this->resource) . ':' . $this->resourceId;
                if (!empty($paramKey)) {
                    $cacheKey .= ':' . $paramKey;
                }
            ...
            $result = $this->retrieveRecordById($this->resource, $this->resourceId, $options);
        ...
        if (!is_null($ids)) {
            ...
            $result = $this->retrieveRecordsByIds($this->resource, $ids, $options);
            ...
            $result = $this->retrieveRecords($this->resource, $records, $options);
        } else {
            if ($this->parent->getCacheEnabled()) {
                // build cache_key
                $cacheKey = $this->resource;
            ...
            $result = $this->retrieveRecordsByFilter($this->resource, $filter, $params, $options);
        }
